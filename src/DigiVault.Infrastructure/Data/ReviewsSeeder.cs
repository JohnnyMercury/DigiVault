...
public static class ReviewsSeeder
{
    ...
    private static readonly (string Title, string Text)[] SteamWalletPositive =
    {
        ("Закинул, всё пришло", "Закинул 1000, на стиме оказалось 1100. По времени минут 7 от оплаты до зачисления, оператор в тг четкий."),
        ("Бонус реальный", "Сначала думал что подвох с +10%. Никакого подвоха, отдал 500р забрал 550р на стим."),
        ("стандартно работает", "Третий раз пополняю. Цены норм, бонус радует, оператор отвечает быстро."),
        ("По времени быстро", "Оплатил картой, написал в телеграм с номером заказа, через 6 минут уже играл с пополнением."),
        ("Цена топ", "Сравнил с другими сервисами, тут с бонусом получается выгоднее процентов на 12-15."),
        ("Норм, рекомендую", "Закинул 2500 на стим за 2200 что ли, разница ощутимая. Деньги пришли в течение 10 минут."),
        ("Закидываю регулярно", "Уже примерно раз в месяц-два пополняю баланс. Никогда не было проблем, оператор всегда на связи."),
        ("спасибо", "пополнил 5к за 4500. бонус +10 это ровно то что обещают, все четко."),
        ("Все по описанию", "Указал свой логин стим, оплатил, написал в тг номер заказа. Через 8 минут пришло уведомление о пополнении."),
        ("Вкатился впервые", "Боялся брать но решил рискнуть на 300р. Все пришло за 5 минут с бонусом. Теперь буду пользоваться."),
        ("Берет адекватно", "Не скрывает что есть проверка по безопасности, так и есть. В тг сразу пишут как только подтвердят."),
        ("Удобный слайдер", "На странице видно сразу сколько отдашь и сколько получишь. Не надо самому считать."),
        ("Быстро и недорого", "1500р закинул, 1650 пришло. Задержка минут 5-7, нормально для ручной проверки."),
        ("Чисто, без подвохов", "Никаких скрытых комиссий или довесков. Что обещали то и пришло. Логин с пароля не спрашивали."),
        ("Зачисление через тг", "Сначала непонятно зачем тг, но потом понял что это часть процесса. Менеджер реально подтверждает заказ и зачисляет."),
        ("ок норм", "пополнил 200р, через 6 минут пришло 220р. для проверки сервиса самое то"),
        ("Закидываю сыну", "Сын играет в кс, регулярно нужны пополнения. Тут и бонус есть и оператор всегда отвечает."),
        ("Цены лучшие", "Сравнивал с тремя другими сервисами по 1000р. Здесь самый ощутимый бонус, плюс быстро."),
        ("Все ок", "Оплатил через сбп, в тг написал, через несколько минут уже на счету в стиме лежали деньги с бонусом."),
        ("Закрыл сразу", "Не люблю долго ждать. Заказ закрыли минут за 6 после того как написал номер в телеграм."),
    };
    ...
    private static readonly (string Title, string Text)[] RobuxPositive =
    {
        ...
        ("Выгодно", "В самой игре покупать дороже получается. Тут экономия процентов 20"),
        ...
        ("Отлично", "400 Robux пришли за минуту после оплаты."),
        ("Рекомендую", "Лучше чем переплачивать в самой игре. Буду возвращаться"),
        ("спс", "Robux прилетели за пару минут, все норм"),
    };
    ...
    private static readonly (string Title, string Text)[] FourStarTexts =
    {
        ...
        ("Хороший сервис", "Все работает, цены адекватные. Минус что нет оплаты через баланс телефона"),
    ...
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        var steamWallet = giftCards.FirstOrDefault(g => g.Slug == "steam-wallet");
        if (steamWallet != null) AddReviews(38, SteamWalletPositive, giftCardId: steamWallet.Id);
